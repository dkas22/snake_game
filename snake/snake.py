...
# game_font = pygame.font.Font(None, font_size) # None for default font

main_game = MAIN()

SCREEN_UPDATE = pygame.USEREVENT
pygame.time.set_timer(SCREEN_UPDATE, 60) # 150 is in mili second, we can use it as level of game
...
while not finished:
	for event in pygame.event.get():
		if event.type == pygame.QUIT:
			pygame.quit()  #this will quit the game
			sys.exit()
		...
		if event.type == pygame.KEYDOWN:
			if event.key == pygame.K_UP:
				...
				if main_game.snake.direction.x != 1:
					main_game.snake.direction = Vector2(-1, 0)
	screen.fill((170,215,70)) #this is rgb color method
